...
class GenerateZone {
    constructor(cassandra, zoneinfo, refreshSerial) {
	    return new Promise((resolve, reject) => {
		    this._log = require('wraplog')('generate-zone');
		    let self = this;
		    ...
		    cassandra.execute(getNsPools, [], cst.readConsistency())
			.then((nsp) => {
				if (nsp.rows !== undefined && nsp.rows[0] !== undefined) {
				    ...
				    cassandra.execute(listRecordsFrom, [ zoneinfo.origin ], cst.readConsistency())
					.then(record => {
						...
						if (record.rows !== undefined) {
						    ...
						    record.rows.forEach(function(rec) {
							    waitRecords.push(new Promise((resolveDeep, rejectDeep) => {
									...
									cassandra.execute(checkCond, [], cst.readConsistency())
									    .then(conditions => {
											...
											if (conditions.rows.length > 0) {
											    ...
											    cassandra.execute(isHealthy, [], cst.readConsistency())
												.then(check => {
									...
									.then((signout) => {
										...
										return execAsync('cmp ' + tmpFile + ' ' + dstFile)
										    .then((cmpout) => {
											    ...
											    cassandra.execute(getSigned, [], cst.readConsistency())
												.then((resp) => {
										    ...
										    .catch((e) => {
											    ...
											    cassandra.execute(insertZone, [ signedZone ], cst.writeConsistency())
												.then((insertResp) => {
													...
													execAsync(gensigs, { cwd: nsKeysDir })
													    .then((gen) => {
															...
															cassandra.execute(insertDS, [ dsRec ], cst.writeConsistency())
															    .then((dsret) => {
