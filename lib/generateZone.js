...
const defaultNs = process.env.NS || 'ns1.unetresgrossebite.com';
const defaultNsSoa = process.env.SOA_NS || 'ns1.unetresgrossebite.com.';
const defaultContactSoa = process.env.SOA_CONTACT || 'samuel.unetresgrossebite.com.';
...
class GenerateZone {
    constructor(cassandra, zoneinfo, refreshSerial) {
	    return new Promise((resolve, reject) => {
		    ...
		    let self = this;
		    this._log.info('generating zone ' + zoneinfo.origin + ' owned by ' + zoneinfo.idowner + ' orig serial: ' + zoneinfo.serial + ' refresh=' + refreshSerial);
		    ...
		    let generateHeader = function(infos) {
			    let ret = [], nsList;
			    ret.push('$TTL 259200');
			    ret.push('@ SOA ' + defaultNsSoa + ' ' + defaultContactSoa + ' (');
			    ret.push('		' + serial);
			    ...
			    if (infos.ns !== undefined && infos.ns !== null) { nsList = infos.ns.split(' '); }
			    else { nsList = defaultNs.split(' '); }
			    for (let i = 0; i < nsList.length; i++) {
				if (nsList[i].match(/\.$/)) { ret.push('@ IN NS ' + nsList[i]); }
				else { ret.push('@ IN NS ' + nsList[i] + '.'); }
			    }
		    ...
		    if (process.env.DEBUG) { self._log.info('running with ' + zoneinfo.origin); }

		    this._buffer = generateHeader(zoneinfo);
				...
				if (record.rows !== undefined) {
							...
							cassandra.execute(checkCond)
							    .then(conditions => {
									...
									if (conditions.rows.length > 0) {
										...
										.catch((e) => {
											self._log.info('failed querying cassandra about check UUID ' + rec.healthcheckid);
											self._log.info(e);
											resolveDeep(true);
										    });
							    ...
							    .catch((e) => {
								    self._log.info('failed looking up health check conditions for UUID ' + rec.healthcheckid);
								    self._log.info(e);
								    resolveDeep(true);
								});
						    }
					...
					.then(() => {
						...
						return execAsync(cmdSelector.checkzone + zoneinfo.origin + ' ' + tmpFile)
						    ...
						    .catch((e) => { self._log.info('zone check failed for ' + zoneinfo.origin); throw new Error('zone check failed'); });
					    })
					 .then(() => {
						if (zoneinfo.zsk !== undefined && zoneinfo.zsk !== null && zoneinfo.ksk !== undefined && zoneinfo.ksk !== null) {
							...
							.then((signout) => {
								...
								return execAsync('cmp ' + tmpFile + ' ' + dstFile)
								    .then((cmpout) => {
											...
											if (resp.rows !== undefined) {
												...
												.catch((e) => {
													...
													return execAsync(installCommand)
													    ...
													    .catch((installerr) => {
														    self._log.info('failed installing signed zone from cassandra for ' + zoneinfo.origin);
														    reject(false);
														});
												    });
											} else {
										...
										.catch((e) => {
											self._log.info('failed fetching signed zone from cassandra');
											reject(false);
										    });
									})
								    .catch((e) => {
										...
										.then((insertResp) => {
											...
											execAsync(gensigs, { cwd: nsKeysDir })
											    .then((gen) => {
													...
													let insertDS = "UPDATE dsrecords SET ds = ? WHERE origin = '" + zoneinfo.origin + "' AND idowner = '" + zoneinfo.idowner + "'";
													    ...
													    .then((dsret) => {
															...
															.then((installout) => {
																if (refreshSerial === true) {
																    ...
																    let command = "UPDATE zones SET serial = '" + serial + "' WHERE idowner = '" + zoneinfo.idowner + "' AND origin = '" + zoneinfo.origin + "'";
																	...
																	.catch((e) => {
																		self._log.info('failed updating serial for ' + zoneinfo.origin + ' with: ' + JSON.stringify(e));
																		reject(false);
																	    });
																} else {
															...
															.catch((e) => {
																self._log.info('failed installing new database for ' + zoneinfo.origin);
																reject(false);
															    });
														})
													    .catch((eds) => {
														    self._log.info('failed inserting DS record for ' + zoneinfo.origin);
														    self._log.info(e);
														    reject(false);
														});
												    } else {
													self._log.info('missing DS record for ' + zoneinfo.origin);
													self._log.info(e);
													reject(false);
												    }
												})
											    .catch((esig) => {
												    self._log.info('failed extracting DS record for ' + zoneinfo.origin);
												    self._log.info(e);
												    reject(false);
												});
										    })
										.catch((e) => {
											self._log.info('failed inserting signed zone data for ' + zoneinfo.origin);
											self._log.info(e);
											reject(false);
										    });
									});
							...
							.catch((e) => {
								self._log.info('failed signing ' + zoneinfo.origin);
								self._log.info(e);
								reject(false);
							    });
						} else {
								...
								return execAsync('cp -p ' + tmpFile + ' ' + dstFile)
								    .then((installout) => {
										...
										cassandra.execute(command)
										    ...
										    .catch((casserror) => {
											    self._log.info('failed updating serial for ' + zoneinfo.origin + ' with: ' + JSON.stringify(caserror));
											    reject(false);
											});
									    } else {
										self._log.info('installed new database for ' + zoneinfo.origin);
					...
					.catch((e) => {
						self._log.info('not installing zone for ' + zoneinfo.origin);
						self._log.info(e);
						resolve(true);
					    });
				} else {
				    self._log.info('failed listing records, cassandra returned ' + JSON.stringify(record));
				    reject(false);
				}
			    })
			.catch(e => {
				self._log.info('failed generating ' + zoneinfo.origin + ' with error ' + JSON.stringify(e));
				reject(false);
			    });
		});
	}
