...
class GenerateZone {
    constructor(cassandra, zoneinfo, refreshSerial) {
	    return new Promise((resolve, reject) => {
		    ...
		    cassandra.execute(getNsPools, [], { consistency: drv.types.consistencies.localQuorum })
			.then((nsp) => {
				if (nsp.rows !== undefined && nsp.rows[0] !== undefined) {
				    ...
				    cassandra.execute(listRecordsFrom, [ zoneinfo.origin ], { consistency: drv.types.consistencies.localQuorum })
					.then(record => {
						...
						if (record.rows !== undefined) {
						    ...
						    record.rows.forEach(function(rec) {
							    waitRecords.push(new Promise((resolveDeep, rejectDeep) => {
									...
									cassandra.execute(checkCond, [], { consistency: drv.types.consistencies.localQuorum })
									    .then(conditions => {
											...
											if (conditions.rows.length > 0) {
											    ...
											    cassandra.execute(isHealthy, [], { consistency: drv.types.consistencies.localQuorum })
												.then(check => {
									...
									.then((signout) => {
										...
										return execAsync('cmp ' + tmpFile + ' ' + dstFile)
										    .then((cmpout) => {
											    ...
											    cassandra.execute(getSigned, [], { consistency: drv.types.consistencies.localQuorum })
												.then((resp) => {
										    ...
										    .catch((e) => {
											    ...
											    cassandra.execute(insertZone, [ signedZone ], { consistency: drv.types.consistencies.localQuorum })
												.then((insertResp) => {
													...
													execAsync(gensigs, { cwd: nsKeysDir })
													    .then((gen) => {
															...
															cassandra.execute(insertDS, [ dsRec ], { consistency: drv.types.consistencies.localQuorum })
															    .then((dsret) => {
