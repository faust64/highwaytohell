	...
	return new Promise ((resolve, reject) => {
		...
		let checkPerms = function(domain, user, contextPerms) {
			return new Promise ((res, rej) => {
				...
				cassandra.execute(lookupPerms, [], { consistency: drv.types.consistencies.localQuorum })
				    .then((role) => {
		...
		Joi.validate({ userId: userId, accessToken: tokenString }, tokenAuthSchema, ((err, res) => {
			if (err) {
			    ...
			    cassandra.execute(lookupToken, [], { consistency: drv.types.consistencies.localQuorum })
				.then((resp) => {
