		...
		cassandra.execute(lookupHistory)
		    .then((resp) => {
				...
				if (count === limit) {
				    ...
				    let lookupTarget = "SELECT target FROM checks WHERE uuid = '" . notify.idcheck + "'";
				    cassandra.execute(lookupTarget)
					.then((tgt) => {
						if (notify.notifydriver === 'http-get') {
						    let renderUrl = notify.notifytarget.replace(':status:', ((resp.rows[0].value === true) ? 'up' : 'down')).replace(':target:', encodeURIComponent(tgt.target));
						    requestAsync({
								headers: { 'User-Agent': 'HighWayToHell-Notify' },
								url: renderUrl
							    })
							.then((reply) => {
								logger.info('notified ' + notify.notifytarget + ' regarding ' + notify.idcheck + ' / ' + tgt.target);
								resolve(true);
							    })
							.catch((e) => {
