					...
					cassandra.execute(lookupTarget)
					    .then((tgt) => {
							...
							requestAsync({
								    ...
								    url: notify.notifytarget.replace(':status:', ((resp.rows[0].value === true) ? 'up' : 'down')).replace(':target:', encodeURIComponent(tgt.target))
								})
							    .then((reply) => {
							...
							if (notify.notifytarget.indexOf('@') > 0) {
							    ...
							    return new sendMail.SendMail(notify.notifytarget, 'notify', subst)
								.then((ok) => {
									logger.info('notified ' + notify.notifytarget + ' regarding ' + notify.checkid + ' / ' + tgt.target);
									resolve(true);
								    })
