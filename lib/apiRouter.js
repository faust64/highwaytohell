...
const fs = require('fs');
const logger = require('wraplog')('api-router');
...
module.exports = (app, cassandra, confQueues, zonesQueues, notifyQueue) => {
	...
	this._healthJob = schedule.scheduleJob('*/10 * * * * *', () => {
	    /*
	     * FIXME: running health check, we could let ops decide which
	     * consistenty level is right
	     */
	...
	this._getPool = function(domainName) {
		return new Promise ((resolve, reject) => {
			...
			cassandra.execute(lookupPool, [], cst.readConsistency())
			    .then((resp) => {
				    if (resp.rows !== undefined && resp.rows[0] !== undefined) {
