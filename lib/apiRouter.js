					...
					apiRoutes.enable2fa(cassandra, userId)
					    .then((token) => {
							...
							QRCode.toDataURL(token.otpauth_url, function(err, data_url) {
								if (err) {
								    let renderWith = { username: req.session.username, email: req.session.email, userid: userId, twofadata: '<b>failed to enable</b>', logindata: [], contacts: [ { target: req.session.email } ] };
