...
module.exports = (app, cassandra, confQueues, zonesQueues) => {
	...
	this._getPool = function(domainName, userId) {
		return new Promise ((resolve, reject) => {
			let lookupPool = "SELECT nspool, bkppool FROM zones WHERE origin = '" + domainName + "' AND idowner = '" + userId + "'";
			cassandra.execute(lookupPool)
			    .then((resp) => {
				    if (resp.rows !== undefined && resp.rows[0] !== undefined) {
					resolve(resp.rows[0].nspool, resp.rows[0].bkppool);
				    } else {
	...
	app.get('/login', (req, res) => {
	     ...
	     */
		if (req.session.username !== undefined) { res.redirect('/domains'); }
		else { res.send(Mustache.render(templates['login'], {})); }
	    });
				...
				.then((userId) => {
					apiRoutes.addZone(cassandra, userId, domainName)
					    ...
					    .catch((e) => {
						    ...
						    logger.error(e);
						    if (req.session.username !== undefined) {
							let renderWith = { username: req.session.username, errormsg: 'backend error' };
							res.send(Mustache.render(templates['backendError'], renderWith));
						    } else { res.status(500).send('backend error'); }
						});
				    })
					    ...
					    .then((nsPool, bkpPool) => {
							...
							.then((zone) => {
								if (confQueues[nsPool] !== undefined) {
								    logger.info('notifying ' + nsPool + ' refresh workers to reload ' + domainName);
								    confQueues[nsPool].createJob({ origin: domainName }).save();
								} else { logger.info('no queue running for ' + nsPool + ' -- restart REQUIRED!'); }
								if (bkpPool !== nsPool) {
								    if (confQueues[bkpPool] !== undefined) {
									logger.info('notifying ' + bkpPool + ' refresh workers to reload ' + domainName);
									confQueues[bkpPool].createJob({ origin: domainName }).save();
								    } else { logger.info('no queue running for ' + bkpPool + ' -- restart REQUIRED!'); }
								}
								if (req.session.username !== undefined) { res.redirect('/domains'); }
								else { res.send(zone); }
							    })
	...
	app.post('/domains/:domainName/getdnssec', (req, res) => {
		...
		cmdValidation(req, res, 'get-domain')
		    .then((params) => {
			    ...
			    authValidation(cassandra, req, 'zones:ro')
				.then((userId) => {
					apiRoutes.getZoneDS(cassandra, userId, domainName)
					    .then((ds) => {
		...
		cmdValidation(req, res, 'add-notification')
		    .then((params) => {
				...
				.then((userId) => {
					...
					apiRoutes.addNotification(cassandra, domainName, checkId, notObj)
					    .then((checks) => {
						    if (req.session.username !== undefined) { res.redirect('/notifications/' + domainName); }
						    else { res.send(checkId); }
						})
					    .catch((e) => {
						    logger.error('failed adding notification for ' + userId + ' ' + domainName + '/' + checkId);
						    logger.error(e);
						    if (req.session.username !== undefined) {
							let renderWith = { username: req.session.username, errormsg: 'backend error adding notification' };
							res.send(Mustache.render(templates['backendError'], renderWith));
						    } else { res.status(500).send('backend error adding notification'); }
						});
				    })
				.catch((e) => {
					logger.error('failed authenticating user from ' + params.actualIP);
				...
				.then((userId) => {
					...
					apiRoutes.delNotification(cassandra, domainName, checkId)
					    ...
					    .catch((e) => {
						    ...
						    logger.error(e);
						    if (req.session.username !== undefined) {
							let renderWith = { username: req.session.username, errormsg: 'backend error dropping notification' };
							res.send(Mustache.render(templates['backendError'], renderWith));
						    } else { res.status(500).send('backend error dropping notification'); }
						});
				    })
				.catch((e) => {
					logger.error('failed authenticating user from ' + params.actualIP);
		    ...
		    .then((params) => {
				...
				.then((userId) => {
					...
					apiRoutes.addNotification(cassandra, domainName, checkId, notObj)
					    ...
					    .catch((e) => {
						    logger.error('failed editing notification for ' + userId + ' ' + domainName + '/' + checkId);
						    logger.error(e);
						    if (req.session.username !== undefined) {
							let renderWith = { username: req.session.username, errormsg: 'backend error editing notification' };
							res.send(Mustache.render(templates['backendError'], renderWith));
						    } else { res.status(500).send('backend error editing notification'); }
						});
				    })
				.catch((e) => {
					logger.error('failed authenticating user from ' + params.actualIP);
					    ...
					    .then((record) => {
							...
							.then((nsPool, bkpPool) => {
								if (zonesQueues[nsPool] !== undefined) {
								    logger.info('notifying ' + nsPool + ' refresh workers to reload ' + domainName);
								    zonesQueues[nsPool].createJob({ origin: domainName }).save();
								} else { logger.info('no queue running for ' + nsPool + ' -- restart REQUIRED!'); }
								if (nsPool !== bkpPool) {
								    if (zonesQueues[bkpPool] !== undefined) {
									logger.info('notifying ' + bkpPool + ' refresh workers to reload ' + domainName);
									zonesQueues[bkpPool].createJob({ origin: domainName }).save();
								    } else { logger.info('no queue running for ' + bkpPool + ' -- restart REQUIRED!'); }
								}
	...
	app.post('/settings/2fa/confirm', (req, res) => {
		...
		cmdValidation(req, res, '2fa-confirm')
				...
				.then((userId) => {
					apiRoutes.confirm2fa(cassandra, userId, params.confirmation)
					    ...
					    .catch((e) => {
						    ...
						    logger.error(e);
						    let renderWith = { username: username, errormsg: '2fa configuration failed' };
						    res.send(Mustache.render(templates['backendError'], renderWith));
						});
					})
				    .catch((e) => {
		    ...
		    .catch((e) => {
			    ...
			    logger.error(e);
			    if (req.session.username !== undefined) {
				let renderWith = { username: username, errormsg: 'invalid input confirming 2fa configuration' };
				res.send(Mustache.render(templates['backendError'], renderWith));
	...
	app.post('/settings/2fa/enable', (req, res) => {
		...
		cmdValidation(req, res, '2fa-enable')
					...
					apiRoutes.enable2fa(cassandra, userId)
					    .then((token) => {
							...
							QRCode.toDataURL(token.otpauth_url, function(err, data_url) {
								if (err) {
								    let renderWith = { username: req.session.username, email: req.session.email, userid: userId, twofadata: '<b>failed to enable</b>', logindata: [] };
								    res.send(Mustache.render(templates['settings'], renderWith));
								} else {
								    let renderWith = { username: req.session.username, email: req.session.email, userid: userId, twofaurl: data_url, logindata: [] };
					    ...
					    .catch((e) => {
						    ...
						    logger.error(e);
						    if (req.session.username !== undefined) {
							let renderWith = { username: username, errormsg: 'failed enabling 2FA' };
							res.send(Mustache.render(templates['backendError'], renderWith));
						    } else { res.status(500).send('backend error'); }
						});
					})
		    ...
		    .catch((e) => {
			    ...
			    logger.error(e);
			    if (req.session.username !== undefined) {
				let renderWith = { username: username, errormsg: 'invalid input enabling 2fa' };
				res.send(Mustache.render(templates['backendError'], renderWith));
	...
	app.post('/settings/2fa/disable', (req, res) => {
		...
		cmdValidation(req, res, '2fa-disable')
				...
				.then((userId) => {
					apiRoutes.disable2fa(cassandra, userId, params.confirmation)
					    ...
					    .catch((e) => {
						    ...
						    logger.error(e);
						    if (req.session.username !== undefined) {
							let renderWith = { username: username, errormsg: 'failed disabling 2FA' };
							res.send(Mustache.render(templates['backendError'], renderWith));
						    } else { res.status(500).send('backend error'); }
						});
					})
		    ...
		    .catch((e) => {
			    ...
			    logger.error(e);
			    if (req.session.username !== undefined) {
				let renderWith = { username: username, errormsg: 'invalid input disabling 2fa' };
				res.send(Mustache.render(templates['backendError'], renderWith));
