		    ...
		    .then((params) => {
				...
				.then((userId) => {
					apiRoutes.delContact(cassandra, userId, params.contactTarget)
					    .then((resp) => {
						    if (req.session.username !== undefined) {
							res.redirect('/settings/contacts');
						    } else { res.send(resp); }
						})
					    .catch((e) => {
				...
				.then((userId) => {
					apiRoutes.listContacts(cassandra, userId)
					    .then((contacts) => {
						    let retWith = [];
						    for (let k = 0; k < contacts.length; k++) {
							let isActive = (contacts[k].confirmcode !== 'true') ? 'pending confirmation' : 'yes';
		    ...
		    .then((params) => {
				...
				.then((userId) => {
					...
					.then((resp) => {
						...
						let labelFail =  (logFail ? 'Notify on failed logins' : 'Do not notify on failed logins');
						let labelSuccess =  (logSuccess ? 'Notify on successfull logins' : 'Do not notify on successfull logins');
						...
						renderWith.notifyFail = statFail + "<br/><form method=POST action='/settings/notify/login'><input type='hidden' name='logFailure' value='" + (!logFail) + "'/><input type='submit' value='" + labelFail + "'/></form>";
						renderWith.notifySuccess = statSuccess + "<br/><form method=POST action='/settings/notify/login'><input type='hidden' name='logSuccess' value='" + (!logSuccess) + "'/><input type='submit' value='" + labelSuccess + "'/></form>";
						renderWith.twofadata = (resp.enabled === true ? disable2fa : enable2fa);
						apiRoutes.listContacts(cassandra, userId)
						    .then((contacts) => {
							    if (contacts.length > 0) {
								for (let k = 0; k < contacts.length; k++) {
								    if (contacts[k].type === 'smtp' && contacts[k].confirmcode === 'true') {
									renderWith.contacts.push({ target: contacts[k].target });
								    }
								}
							    }
							    if (renderWith.contacts.length === 0) {
								renderWith.contacts.push({ target: req.session.email });
							    }
							    apiRoutes.listLogins(cassandra, userId)
								.then((loginhistory) => {
