...
function pullDnssecKeys() {
    return new Promise((resolve, reject) => {
	    client.execute('SELECT * FROM zones', [], { consistency: cassandra.types.consistencies.localQuorum })
		.then((resp) => {
...
zonesSub.on('message', (chan, msg) => {
	...
	client.execute(lookupDomain, [ msg ], { consistency: cassandra.types.consistencies.localQuorum })
	    .then((resp) => {
		    if (resp.rows !== undefined) {
...
zonesQueue.process((task, done) => {
	if (task.data.origin !== undefined) {
	    client.execute(lookupDomain, [ task.data.origin ], { consistency: cassandra.types.consistencies.localQuorum })
		.then((dom) => {
