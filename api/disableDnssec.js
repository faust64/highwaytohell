...
module.exports = (cassandra, domain) => {
	return new Promise ((resolve, reject) => {
		...
		cassandra.execute(getZone, [], { consistency: drv.types.consistencies.one })
		    .then((keys) => {
			    let purgeKeys = "DELETE FROM dnsseckeys WHERE ksk = '" + keys.rows[0].ksk + "' AND zsk = '" + keys.rows[0].ksk + "'";
			    cassandra.execute(disableDnssec, [], { consistency: drv.types.consistencies.one })
				.then((resp) => {
					cassandra.execute(purgeKeys, [], { consistency: drv.types.consistencies.one })
					    .then((purge) => { resolve({}); })
